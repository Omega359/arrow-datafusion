...
/// test harness.  Each benchmark is defined by a file that can contain
/// directives such as `load`, `run`, `assert`, `result`, etc.  The
/// `SqlBenchmark` struct holds the parsed data from that file and
/// provides methods to run, assert, persist, and verify benchmark
/// results.
#[derive(Debug, Clone)]
...
impl SqlBenchmark {
    ...
    fn compare_results(
        ...
        for ((row_idx, actual), expected) in zipped {
            ...
            for (col_idx, expected_val) in expected.iter().enumerate().take(query.column_count) {
...
/// - `ResultQueryOrResult`: Handles `result` and `result_query` directives for
///   persisting and verifying query results
...
/// - `BenchmarkQualifier`: An enum wrapper that delegates to the appropriate
///   handler based on the directive type
///
/// # Error Handling
///
...
/// # Thread Safety
///
/// Implementations should be stateless or use interior mutability safely, as
/// processors may be reused across multiple benchmark files in concurrent
/// execution scenarios.
trait LineProcessor {
    async fn process(
        &self,
