...
impl Default for VectorizedOperationBuffers {
    fn default() -> Self {
        Self {
            ...
            equal_to_results: BooleanBufferBuilder::new(64),
...
impl VectorizedOperationBuffers {
    fn clear(&mut self) {
        ...
        self.equal_to_results = BooleanBufferBuilder::new(64);
...
impl<const STREAMING: bool> GroupValuesColumn<STREAMING> {
    ...
    fn vectorized_equal_to(&mut self, cols: &[ArrayRef], groups: &mut [usize]) {
        let equal_to_groups_indices_len = self
            .vectorized_operation_buffers
            .equal_to_group_indices
            .len();

        assert_eq!(
            equal_to_groups_indices_len,
        ...
        // 1. Perform `vectorized_equal_to` for `rows` in `vectorized_equal_to_group_indices`
        //    and `group_indices` in `vectorized_equal_to_group_indices`
        self.vectorized_operation_buffers.equal_to_results =
            BooleanBufferBuilder::new(equal_to_groups_indices_len);
        let mut equal_to_results = BooleanBufferBuilder::new(equal_to_groups_indices_len);
        equal_to_results.append_n(equal_to_groups_indices_len, true);
