...
use arrow::array::{
    make_view, Array, ArrayRef, AsArray, BooleanBufferBuilder, ByteView,
...
mod tests {
        ...
        ),
    {
        ...
        // Check
        let mut equal_to_results = BooleanBufferBuilder::new(input_array.len());
        equal_to_results.append_n(input_array.len(), true);
        ...
        assert!(equal_to_results.get_bit(6));
        assert!(!equal_to_results.get_bit(7));
        assert!(!equal_to_results.get_bit(8));
        assert!(equal_to_results.get_bit(9));
        assert!(!equal_to_results.get_bit(10));
        assert!(equal_to_results.get_bit(11));
    }
