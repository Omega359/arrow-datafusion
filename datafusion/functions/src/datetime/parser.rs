...
/// but is measurably faster than Chrono.
///
            ...
            for &format in formats.iter() {
                ...
                match dt {
                    ...
                    Err(e) if e.kind() == ParseErrorKind::OutOfRange => {
                        err = Some(format!("Unable to parse timestamp {datetime_str} in timezone {tz:?}: datetime was out of range"));
                        continue;
                    }
                    _ => {
                        ...
                        match result {
                            ...
                            LocalResult::None => {
                                err = Some(format!("Unable to parse timestamp {datetime_str} in timezone {tz:?}: no valid local time found"));
                                continue;
                            }
                            LocalResult::Ambiguous(earliest, latest) => {
                                err = Some(format!("Unable to parse timestamp {datetime_str} in timezone {tz:?}: ambiguous timestamps found {earliest:?} and {latest:?}"));
                                continue;
                            }
                        }
                    }
