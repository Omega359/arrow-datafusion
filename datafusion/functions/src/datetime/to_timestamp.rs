...
use datafusion_common::format::{DEFAULT_CAST_OPTIONS, DEFAULT_FORMAT_OPTIONS};
        ...
        match args[0].data_type() {
            Int32 | Int64 => match self.safe {
                true => args[0]
                    ...
                    .cast_to(&Timestamp(Nanosecond, None), Some(&SAFE_CAST_OPTIONS)),
                false => args[0]
                    .cast_to(&Timestamp(Second, None), None)?
                    .cast_to(&Timestamp(Nanosecond, None), None),
            },
            Null | Timestamp(_, None) => match self.safe {
                true => args[0]
                    .cast_to(&Timestamp(Nanosecond, None), Some(&SAFE_CAST_OPTIONS)),
